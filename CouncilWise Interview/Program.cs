    ...
    */
    class Program
    {
        ...
        static Receipt ProcessReceiptItems(ICollection<ReceiptItem> items)
        {
            ...
            {
                item.TaxAmount = isPalindrome(item.Name) == true ? item.UnitPrice = 0: item.IncludesTax == true ? (item.UnitPrice / 11) : item.UnitPrice / 10;
                
                receipt.Items.Add(item);
            }
            receipt.Total = receipt.Items.Sum(x => (x.UnitPrice + x.TaxAmount) * x.Quantity);
            receipt.TaxTotal = receipt.Items.Sum(x => x.TaxAmount * x.Quantity);
            return receipt;
        }
